...
class Many(unittest.TestCase):
    ...
    def test_nondimensional_energy_minimization_no_hessian(self):
        ...
        self.assertTrue(np.abs((beta_U - beta_U_check)/beta_U_check) < 1e-3)
        self.assertTrue((np.abs((s - s_check)/s_check) < 5e-2).all())
    ...
    def test_nondimensional_energy_minimization_no_hessian_ts(self):
        ...
        time_1 = time()
        s_vec_guess = np.ones(model.L*model.W - (transition_state is not None))
        res = minimize(
